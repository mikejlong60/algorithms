...
func TestBFSRoot7(t *testing.T) {
	...
	graph, rng := g(rng)
	tree, hasCycle := BFSearch(graph, 7)
	expected := [][]Edge{{{-1, 7}}}
...
func TestBFSRoot6(t *testing.T) {
	...
	rng := propcheck.SimpleRNG{now}
	graph, rng := g(rng)
	tree, hasCycle := BFSearch(graph, 6)
...
func TestBFSRoot5(t *testing.T) {
	...
	rng := propcheck.SimpleRNG{now}
	graph, rng := g(rng)
	tree, hasCycle := BFSearch(graph, 5)
...
func TestBFSRoot4(t *testing.T) {
	...
	rng := propcheck.SimpleRNG{now}
	graph, rng := g(rng)
	tree, hasCycle := BFSearch(graph, 4)
	expected := [][]Edge{{{-1, 4}}}
	if !arrays.ArrayEquality(tree, expected, BFSEquality) {
		t.Errorf("Actual:%v Expected:%v", tree, expected)
	}
...
func TestBFSRoot3(t *testing.T) {
	...
	rng := propcheck.SimpleRNG{now}
	graph, rng := g(rng)
	tree, hasCycle := BFSearch(graph, 3)
...
func TestBFSRoot2(t *testing.T) {
	...
	rng := propcheck.SimpleRNG{now}
	graph, rng := g(rng)
	tree, hasCycle := BFSearch(graph, 2)
...
func TestBFSRoot1(t *testing.T) {
	...
	rng := propcheck.SimpleRNG{now}
	graph, rng := g(rng)
	tree, hasCycle := BFSearch(graph, 1)

...
type Node2 struct {
	Id          int
	Connections []NodeDistanceTuple
}

type NodeDistanceTuple struct {
	node     *Node2
	distance int
}
...
func DijkstraSearch2(graph map[int]*Node2, rootId int) [][]chapter3.Edge {
	...
	for {
		...
		for _, k := range tree[i] {
			...
			for _, m := range node.Connections {
				...
				_, alreadySeen := layersLookup[m.node.Id]
				totalSteps2 = totalSteps2 + 1
				if !alreadySeen {
					pendingLayer = append(pendingLayer, chapter3.Edge{U: k.V, V: m.node.Id})
					layersLookup[m.node.Id] = chapter3.NodeLayerTuple{Id: m.node.Id, Layer: i + 1}
				}
			}
		}
...
func DirectedGraphGen2(lower, upperExc int) func(propcheck.SimpleRNG) (propcheck.Pair[map[int]*Node2, int], propcheck.SimpleRNG) {
	return func(rng propcheck.SimpleRNG) (propcheck.Pair[map[int]*Node2, int], propcheck.SimpleRNG) {
		...
		graph := make(map[int]*Node2, len(nodeIds))
		for _, j := range nodeIds {
			graph[j] = &Node2{Id: j}
		}

		var connectionIds []int
		var rng3, rng4 propcheck.SimpleRNG
		for _, node := range graph {
			...
			for _, connectedNodeId := range connectionIds {
				if node.Id != graph[nodeIds[connectedNodeId]].Id {
					var distance int
					distance, rng4 = propcheck.ChooseInt(0, 100)(rng4)
					node.Connections = append(node.Connections, NodeDistanceTuple{node: graph[nodeIds[connectedNodeId]], distance: distance})
				}
			}
		}
		root, rng5 := propcheck.ChooseInt(0, len(graph))(rng4)
		return propcheck.Pair[map[int]*Node2, int]{graph, nodeIds[root]}, rng5
	}
}

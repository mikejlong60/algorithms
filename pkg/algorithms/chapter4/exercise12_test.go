...
func TestMakeScheduleWhereFirstStreamFinishesWithinTimeWindow(t *testing.T) {
	...
	makeSchedule := func(xs []Stream, maxBitsPerSecond int) []Stream {
		var r = []Stream{}
		for a, b := range xs {
			if b.NumberOfBits*b.Duration <= maxBitsPerSecond*b.Duration {
				r = append(r, b)
				r = append(r, xs[0:a]...)
				r = append(r, xs[a+1:len(xs)]...)
				return r
			}
		}
		return []Stream{}

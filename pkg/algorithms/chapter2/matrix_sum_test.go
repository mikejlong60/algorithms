...
func TestFoldLeftMatrixSum(t *testing.T) { //This reverses the order because foldRight does that. There is a Reverse function in Golangz if that matters to you.
	g0 := propcheck.ChooseInt(1, 3000)
	g1 := propcheck.ChooseArray(0, 10000, g0)
	now := time.Now().Nanosecond()
	rng := propcheck.SimpleRNG{now}

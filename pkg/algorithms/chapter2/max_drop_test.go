...
func TestMaxDropGivenBudget(t *testing.T) { //This reverses the order because foldRight does that. There is a Reverse function in Golangz if that matters to you.
	...
	rng := propcheck.SimpleRNG{197695000} //now}
	fmt.Println(rng)
	...
	prop := propcheck.ForAll(g2,
		...
		func(highestWrungAEtAll propcheck.Pair[int, propcheck.Pair[[]int, int]]) (bool, error) {
			var errors error
			indexOfHighestWrung := highestWrungAEtAll.B.B - 1
			ladder := highestWrungAEtAll.B.A
			highestWrung := highestWrungAEtAll.A
			if highestWrung != ladder[indexOfHighestWrung] {
				errors = multierror.Append(errors, fmt.Errorf("Expected highest non-breaking wrung withing budget to be:%v but was:%v", highestWrung, ladder[indexOfHighestWrung]))
			}
			if errors != nil {
				return false, errors
			} else {
